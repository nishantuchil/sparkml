...
public class CustomerSubscriptionPrediction {
  ...
  public static void main(String args[]) {
    ...
    filterDataSet =
        filterDataSet.withColumn("label", when(col("label").isNull(), 0).otherwise(col("label")));
    // filterDataSet.show(100);
    ...
    // testData = stringIndexer.fit(testData).transform(testData);
    // testData.show();
    ...
    // testData = oneHotEncoder.fit(testData).transform(testData);
    // testData.show();
    ...
    // Dataset<Row> transformedData = vectorAssembler.transform(testData);
    ...
    //    (LinearRegressionModel) trainValidationSplit.fit(transformedData).bestModel();
    // LinearRegressionModel lrm = linearRegression.fit(transformedData);
    ...
    LinearRegressionModel lrm = (LinearRegressionModel) model.bestModel();

    System.out.println("Root mean squared error = " + lrm.summary().meanSquaredError());
    System.out.println("R2 = " + lrm.summary().r2());

    Dataset<Row> predictedData = pipelineModel.transform(holdOutData);
    predictedData.select(col("label"), col("prediction")).show(50);

    /*
    LinearRegressionSummary evaluationResult = lrm.evaluate(holdOutData);

    System.out.println("Mean squared error = " + evaluationResult.meanSquaredError());
    System.out.println("R2 = " + evaluationResult.r2());
    */
